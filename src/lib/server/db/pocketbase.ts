...
export class PocketBaseAdapter implements DBAdapter {
	...
	async createBlackjackGame(
		...
		const record = await this.pb.collection("blackjack_games").create({
			...
			player_second_hand: state.player_second_hand
				? JSON.stringify(state.player_second_hand)
				: null,
	...
	async updateBlackjackGame(
		...
		if (state.dealer_hand !== undefined)
			data.dealer_hand = JSON.stringify(state.dealer_hand);
		if (state.player_hand !== undefined)
			data.player_hand = JSON.stringify(state.player_hand);
		if (state.player_second_hand !== undefined)
			data.player_second_hand = state.player_second_hand
				? JSON.stringify(state.player_second_hand)
				: null;
		if (state.player_coins !== undefined)
			data.player_coins = state.player_coins;
		if (state.current_wager !== undefined)
			data.current_wager = state.current_wager;
		...
		if (state.player_stood !== undefined)
			data.player_stood = state.player_stood;
		...
		if (state.current_hand !== undefined)
			data.current_hand = state.current_hand;
		if (state.dealer_value !== undefined)
			data.dealer_value = state.dealer_value;
	...
	async getBlackjackGame(gameId: string): Promise<BlackjackState> {
		...
		return {
			...
			player_second_hand: record.player_second_hand
				? JSON.parse(record.player_second_hand)
				: null,
