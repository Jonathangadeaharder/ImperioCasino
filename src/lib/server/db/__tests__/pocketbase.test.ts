...
function setup() {
	...
	const mockFilter = vi.fn((_template: string, _params: Record<string, unknown>) => `username = "${_params.username}"`);
	const mockPb = {
		collection: vi.fn().mockReturnValue({ getOne: mockGetOne, getList: mockGetList, create: mockCreate, update: mockUpdate }),
		filter: mockFilter
	};
	...
	return { adapter, mockGetOne, mockGetList, mockCreate, mockUpdate, mockFilter };
}
...
describe('PocketBaseAdapter', () => {
	...
	describe('getUserByUsername', () => {
		it('returns user when found', async () => {
			...
			expect(user).toEqual({ id: 'u1', username: 'alice', coins: 500 });
			expect(mockFilter).toHaveBeenCalledWith('username = {:username}', { username: 'alice' });
			expect(mockGetList).toHaveBeenCalledWith(1, 1, { filter: 'username = "alice"' });
		});
	...
	describe('createBlackjackGame', () => {
		...
		it('stringifies player_second_hand when present', async () => {
			...
			expect(callArg.player_second_hand).toBe(JSON.stringify(stateWithSplit.player_second_hand));
		});
		...
		it('throws on failure', async () => {
			...
			await expect(adapter.createBlackjackGame('u1', state)).rejects.toThrow('fail');
		});
	});
	...
	describe('updateBlackjackGame', () => {
		it('updates only provided fields', async () => {
			...
			await adapter.updateBlackjackGame('bg1', { game_over: true, message: 'Bust' });

			expect(mockUpdate).toHaveBeenCalledWith('bg1', { game_over: true, message: 'Bust' });
		});
		...
		it('stringifies hand fields', async () => {
			...
			await adapter.updateBlackjackGame('bg1', { player_hand: [{ suit: 'Hearts', name: 'Ace', value: 11 }] });

			const callArg = mockUpdate.mock.calls[0][1];
			expect(callArg.player_hand).toBe(JSON.stringify([{ suit: 'Hearts', name: 'Ace', value: 11 }]));
		});
		...
		it('throws on failure', async () => {
			...
			await expect(adapter.updateBlackjackGame('bg1', { game_over: true })).rejects.toThrow('fail');
		});
	});
	...
	describe('getBlackjackGame', () => {
		...
		it('parses player_second_hand when present', async () => {
			...
			const recordWithSplit = {
				...record,
				player_second_hand: JSON.stringify([{ suit: 'Clubs', name: '5', value: 5 }])
			};
			mockGetOne.mockResolvedValue(recordWithSplit);

			const state = await adapter.getBlackjackGame('bg1');

			expect(state.player_second_hand).toEqual([{ suit: 'Clubs', name: '5', value: 5 }]);
		});
