...
describe('hooks.server handle', () => {
	it('skips auth when building', async () => {
		const event = { request: { headers: { get: vi.fn(() => '') } }, locals: {} } as any;
	...
	it('loads auth cookie from request', async () => {
		const event = { request: { headers: { get: vi.fn(() => 'pb_auth=token123') } }, locals: {} } as any;
		const resolve = vi.fn().mockResolvedValue(mockResolve());
	...
	it('sets user from auth store when valid', async () => {
		...
		const event = { request: { headers: { get: vi.fn(() => 'pb_auth=token') } }, locals: {} } as any;
		const resolve = vi.fn().mockResolvedValue(mockResolve());

		await handle({ event, resolve });

		expect(event.locals.user).toEqual({ id: 'user1', username: 'test', coins: 500 });
	});
	...
	it('sets set-cookie header on response', async () => {
		...
		expect(mockRes.headers.set).toHaveBeenCalledWith('set-cookie', 'pb_auth=session');
	});
});
