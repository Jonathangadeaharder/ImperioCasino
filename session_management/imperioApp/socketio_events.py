    ...
    if metric == 'net_profit':
        ...
        query = db.session.query(
            ...
            (
                ...
                func.coalesce(
                    func.abs(func.sum(func.case((Transaction.transaction_type == 'BET', Transaction.amount), else_=0))), 0
                )
            ).label('net_profit')
        ...
        query = db.session.query(
            ...
            User.username,
            User.coins,
            func.count(Transaction.id).label('total_wins')
        ).outerjoin(Transaction, User.id == Transaction.user_id).filter(
...
def handle_create_multiplayer_room(data):
    ...
    if not is_private:
        socketio.emit('new_room_available', {
            'room_id': room_id,
            'game_type': game_type,
            'host': user_data['username'],
            'players': 1,
...
def handle_get_multiplayer_rooms(data=None):
    ...
    available_rooms = [
        ...
        if not room['private'] and room['status'] == 'waiting'
        and (game_type is None or room['game_type'] == game_type)
    ]

    emit('multiplayer_rooms_list', {
...
def handle_join_chat(data):
    ...
    socketio.emit('user_joined_chat', {
        'username': user_data['username'],
        'room': chat_room
    }, room=f"chat_{chat_room}", skip_sid=request.sid)
