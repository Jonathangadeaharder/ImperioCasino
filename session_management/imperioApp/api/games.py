...
async def spin(
    ...
    locked_user.coins -= 1
    logging.info(
        "User %s has spun. Coins left: %s", locked_user.username, locked_user.coins
    )
    ...
    logging.info(
        "User %s won: %s coins. New balance: %s",
    ...
    return {"stopSegments": stop_segments, "totalCoins": locked_user.coins}
...
async def start_blackjack_game(
    ...
    if game_request.wager is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Wager is required"
        )
    ...
    try:
        ...
        logging.debug(
            f"Start Blackjack Game - Game State: {game_state.get('player_hand', 'N/A')}"
        )

        if status_code != 200:
            raise HTTPException(
                status_code=status_code, detail=game_state.get("message", "Error")
            )
...
async def blackjack_action(
    ...
    try:
        result, status_code = blackjack.player_action(
            current_user, action_request.action, action_request.game_id
        )
        ...
        if status_code != 200:
            raise HTTPException(
                status_code=status_code, detail=result.get("message", "Error")
            )
