...
def redirect_to_imperio():
    ...
    if not token:
        ...
        session["token"] = token
    return redirect(
        f"{app.config['CHERRY_CHARM_URL']}/?username={username}&token={token}"
    )
...
def get_coins(current_user):
    logging.debug(
        "Received getCoins request for user_id: %s with coins: %s",
...
def update_coins(current_user):
    ...
    logging.debug(
        "Received updateCoins request for user_id: %s with coins: %s",
        current_user.username,
        coins,
    )
    ...
    logging.info(
        "Coins successfully updated for user_id: %s to %s coins",
        current_user.username,
        coins,
    )
...
def redirect_to_blackjack():
    ...
    if not token:
        ...
        session["token"] = token
    return redirect(
        f"{app.config['BLACK_JACK_URL']}/?username={username}&token={token}"
    )
