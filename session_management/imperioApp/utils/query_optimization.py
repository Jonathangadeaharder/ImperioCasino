...
def get_top_users_by_coins(limit=100):
    ...
    users = db.session.query(User.id, User.username, User.coins).order_by(User.coins.desc()).limit(limit).all()

    return [{"id": u.id, "username": u.username, "coins": u.coins} for u in users]
...
def get_user_statistics_cached(user_id):
    ...
    stats = (
        db.session.query(
            ...
            .filter(Transaction.transaction_type == TransactionType.BET)
            .label("total_bets"),
            func.coalesce(
                func.sum(func.abs(Transaction.amount)).filter(Transaction.transaction_type == TransactionType.BET), 0
            ).label("total_wagered"),
            func.count(func.distinct(Transaction.id))
            .filter(Transaction.transaction_type == TransactionType.WIN)
            .label("total_wins"),
            func.coalesce(
                func.sum(Transaction.amount).filter(Transaction.transaction_type == TransactionType.WIN), 0
            ).label("total_won"),
...
def invalidate_user_cache(user_id):
    ...
    patterns = [
        ...
        "leaderboard_*",  # Leaderboards might have changed
    ]
...
def get_game_statistics_optimized(user_id, game_type):
    ...
    stats = (
        ...
        .filter(Transaction.user_id == user_id, Transaction.game_type == game_type)
        .first()
    )
    ...
    for table, indexes in critical_indexes.items():
        ...
        for index in indexes:
            if index not in table_indexes:
                log.warning("missing_index", table=table, index=index, message=f"Consider adding index: {index}")
...
def get_table_sizes():
    ...
    try:
        if db.engine.dialect.name == "postgresql":
            ...
            return [{"schema": row[0], "table": row[1], "size": row[2], "bytes": row[3]} for row in result]
        else:
            return []
    except Exception as e:
...
def analyze_query_performance(query_sql, explain=True):
    ...
    try:
        if explain and db.engine.dialect.name == "postgresql":
            result = db.session.execute(text(f"EXPLAIN ANALYZE {query_sql}"))

            plan = [row[0] for row in result]
            return {"query": query_sql, "plan": plan}
        else:
