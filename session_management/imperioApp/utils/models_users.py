...
class User(SQLAlchemyBaseUserTableUUID, Base):
    ...
    __tablename__ = "users"
    __table_args__ = (Index("idx_username_users", "username"),)
...
class BlackjackGameState(Base):
    ...
    __table_args__ = (Index("idx_user_game_state", "user_id", "game_over"),)
    ...
    def to_dict(self):
        ...
        can_double_down = (
            ...
            and not self.double_down
            and not self.split
            and not self.player_stood
        )
        ...
        if not self.game_over and not self.split and len(self.player_hand) == 2:
            try:
                can_split = self.player_hand[0].get("value") == self.player_hand[1].get(
                    "value"
                )
