...
def has_achievement(user_id, achievement_type):
    ...
    return UserAchievement.query.filter_by(user_id=user_id, achievement_id=achievement.id).first() is not None
...
def unlock_achievement(user, achievement_type):
    ...
    user_achievement = UserAchievement(
        user_id=user.id, achievement_id=achievement.id, unlocked_at=datetime.utcnow(), seen=False
    )
    ...
    log.info(
        "achievement_unlocked", user_id=user.id, achievement=achievement_type.value, reward=achievement.reward_coins
    )
...
def check_winning_streak(user):
    ...
    recent_transactions = (
        Transaction.query.filter(
            Transaction.user_id == user.id, Transaction.transaction_type.in_([TransactionType.BET, TransactionType.WIN])
        )
