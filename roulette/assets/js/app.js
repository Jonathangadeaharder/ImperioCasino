...
let wager = 5;
let lastWager = 0;
let bet = [];               // Array of bet objects {amt, type, odds, numbers}
let numbersBet = [];
let previousNumbers = [];
...
// Fetch initial coins from the server
...
function buildBettingBoard(){
	// (Unchanged code building the entire betting layout)
	// We keep all this code the same as before to maintain the same UI and interactions.
	// The difference is we rely on the server for final outcome.

	// Entire block below is the original unchanged code for building the betting board, chips, etc.
	// --------------------- START OF ORIGINAL BETTING BOARD BUILD CODE ---------------------
	...
	for(let i = 0; i < numberBlocks.length; i++){
		...
		numberBlock.onclick = function(){
			...
			if(numberBlocks[a] != '2 a 1'){
				num = ''+numberBlocks[a]+'';
				setBet(this, num, 'inside_whole', 35);
			}else{
	...
	// --------------------- END OF ORIGINAL BETTING BOARD BUILD CODE ---------------------
}
...
function setBet(e, n, t, o){
	...
	if(wager > 0){
		...
		bankValue = bankValue - wager; // local decrement to show immediate effect
	...
	fetch(`${serverAddress}/roulette/action`, {
		...
		.then(data => {
			if(data.winning_number !== undefined){
				let winningSpin = data.winning_number;
				let total_bet = data.total_bet;
				let total_win = data.total_win;
				let new_coins = data.new_coins;

				// Animate wheel with the winning number
				spinWheel(winningSpin);

				// After the wheel animation ends (10s), show results and update UI
				setTimeout(function(){
					// Update bankValue from server response
					bankValue = new_coins;
					currentBet = 0;
					document.getElementById('bankSpan').innerText = '' + bankValue.toLocaleString("en-GB") + '';
					document.getElementById('betSpan').innerText = '' + currentBet.toLocaleString("en-GB") + '';
					...
					// Display win notification if player won
					win(winningSpin, total_win, total_bet);
		...
		.catch(err => console.error('Error spinning:', err));
}
...
function win(winningSpin, winValue, betTotal){
	if(winValue > 0){
		...
		nsWinBlock = document.createElement('div');
		nsWinBlock.setAttribute('class', 'nsWinBlock');
		nsWinBlock.innerText = 'Payout: ' + (winValue);
		nsWin.append(nsWinBlock);
		nSpan.append(nsWin);
		notification.append(nSpan);
		container.prepend(notification);
	...
	for(let i = 0; i < bet.length; i++){
		if(bet[i].numbers == n && bet[i].type == t){
			if(bet[i].amt != 0){
				...
				bet[i].amt = bet[i].amt - wager;
				bankValue = bankValue + wager; // local refund
				currentBet = currentBet - wager;
				document.getElementById('bankSpan').innerText = '' + bankValue.toLocaleString("en-GB") + '';
				document.getElementById('betSpan').innerText = '' + currentBet.toLocaleString("en-GB") + '';
				if(bet[i].amt == 0){
...
function spinWheel(winningSpin){
	// same animation logic as before, but we now have a predetermined winningSpin from the server
	let degree;
	...
	setTimeout(function(){
		wheel.style.cssText = '';
		// style is removed after finishing
		let styles = document.querySelectorAll('head style');
		styles.forEach(st=>{
			if(st.innerText.includes('@keyframes ballStop')) st.remove();
		});
	}, 10000);
}
